...
flags.DEFINE_integer("steps", 10000, "Training batch size")

FLAGS = flags.FLAGS
TRAIN_FILE = "train.tfrecords"

def get_global_step(graph=None):
  "Extracts the global step from the model"
...
def main(unused_argv):
  ...
  with tf.device(device):
    ...
    evaluation_op = mnist.evaluation(logits, labels)

  sv = tf.train.Supervisor(logdir=FLAGS.train_dir, is_chief=(FLAGS.task_index == 0))

  step = 0
  with sv.managed_session(target) as sess:
    global_step_op = get_global_step(sess.graph)
    while not sv.should_stop() and step < FLAGS.steps:
      _, step, evaluation = sess.run([train_op, global_step_op, evaluation_op])
      print("Global step %d, evaluation: %f" % (int(step), float(evaluation)))
      sys.stdout.flush()
